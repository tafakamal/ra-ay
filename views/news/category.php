 ...
 *   $category    - array of current category details
...
// Helper to generate page URL preserving category slug
$pageUrl = function(int $pageNum) use ($category): string {
    return '/berita/kategori/' . e($category['slug']) . '?page=' . $pageNum;
};
?>
...
<section class="section py-12 bg-surface">
  <div class="container">
    <div data-animate="fade-in">
      <span class="text-primary font-semibold uppercase tracking-wider text-sm block mb-2">Arsip Berita</span>
      <h1 class="text-4xl font-extrabold tracking-tight text-fg mb-4">
        Kategori: <span style="color: <?= e($category['color'] ?? 'var(--primary)') ?>;"><?= e($category['name']) ?></span>
      </h1>
      <?php if (!empty($category['description'])): ?>
        <p class="text-lg text-muted max-w-2xl"><?= e($category['description']) ?></p>
      <?php else: ?>
        <p class="text-lg text-muted max-w-2xl">Menampilkan semua berita dan artikel di bawah kategori <?= e($category['name']) ?>.</p>
      <?php endif; ?>
    </div>
  </div>
</section>
      ...
      <div class="lg:col-span-3">
        <?php if (empty($articles)): ?>
          <div class="text-center py-16 bg-card border rounded" data-animate="slide-up">
            ...
            <h3 class="text-lg font-bold text-fg">Belum Ada Berita</h3>
            <p class="text-fg-muted mt-2">Kategori ini saat ini belum memiliki berita yang diterbitkan.</p>
        ...
        <?php else: ?>
            ...
            <?php foreach ($articles as $news): ?>
              <article class="card card-news" data-animate="slide-up">
                ...
                <div class="card__body">
                  <!-- Category Badge -->
                  <span class="card__tag" style="background-color: <?= e($category['color'] ?? '#FFF0E5') ?>20; color: <?= e($category['color'] ?? '#E8600A') ?>;">
                    <?= e($category['name']) ?>
                  </span>
          ...
          <!-- Pagination -->
          <?php if ($totalPages > 1): ?>
            <nav class="pagination" aria-label="Navigasi Halaman Berita Kategori" data-animate="fade-in">
              <!-- Previous Button -->
              ...
              <?php foreach ($categories as $cat): ?>
                <li>
                  <a href="/berita/kategori/<?= e($cat['slug']) ?>" 
                     class="flex justify-between items-center px-3 py-2 rounded text-sm font-medium transition <?= $cat['id'] === $category['id'] ? 'bg-primary text-white hover:bg-primary' : 'hover:bg-primary-light hover:text-primary' ?>">
                    <span class="flex items-center gap-2">
                      <span style="width: 8px; height: 8px; border-radius: 50%; background-color: <?= $cat['id'] === $category['id'] ? '#FFF' : e($cat['color'] ?? '#E8600A') ?>;"></span>
                      <?= e($cat['name']) ?>
                    </span>
                    <span class="text-xs px-2 py-0.5 rounded-full <?= $cat['id'] === $category['id'] ? 'bg-white text-primary' : 'text-muted bg-surface' ?>"><?= $cat['news_count'] ?></span>
                  </a>
                </li>
              <?php endforeach; ?>
            </ul>
